...
| Pricing engine, services, IPC, and UI                          | Present in `dev`                              | Present on `integration/option`          | Implemented modules exist, but IPC naming, command registration, and UI reachability gaps keep M9 unaccepted; Option plugins are not present |
