...
| SQL schema file                                                | Present as `0004_options_support.sql`         | Present                                  | Not integrated: the current migration runner does not apply it                       |
| Repositories                                                   | Present under `database/repositories`         | Present                                  | Implemented, but runtime schema and repository tests are incomplete                  |
| TypeScript protocol types                                      | Present in `apps/desktop/src/types/option.ts` | Extended                                 | Implemented on `dev`; external inputs still require schema validation                |
| Pricing engine, services, IPC, UI, and Option plugins          | Absent                                        | Present on `integration/option`          | Candidate code only; not part of `dev` and not verified against current architecture |
| End-to-end Option workflow                                     | Absent                                        | Claimed complete by the candidate branch | Must be independently audited and revalidated                                        |
