    ...
    let mut matches: Vec<SearchMatch> = chunks
        ...
        .filter_map(|chunk| {
            ...
            let score = query_terms
                .iter()
                .map(|query_term| score_term(&terms, query_term))
                .sum();
            (score > 0).then(|| SearchMatch {
                ordinal: chunk.ordinal,
                content: chunk.content.clone(),
...
fn score_term(chunk_terms: &[String], query_term: &str) -> usize {
    let exact = chunk_terms
        .iter()
        .filter(|term| term.as_str() == query_term)
        .count();
    let stem_matches = chunk_terms
        .iter()
        .filter(|term| stem(term) == stem(query_term) && term.as_str() != query_term)
        .count();
    let related = related_terms(query_term)
        .iter()
        .map(|related| {
            chunk_terms
                .iter()
                .filter(|term| term.as_str() == *related)
                .count()
        })
        .sum::<usize>();
...
mod tests {
    ...
    fn ranks_matching_chunks_case_insensitively() {
        let chunks = vec![
            TextChunk {
                ordinal: 0,
                content: "AI data center demand".into(),
            },
            TextChunk {
                ordinal: 1,
                content: "AI AI infrastructure".into(),
            },
        ];
        assert_eq!(
            rank_chunks(&chunks, "ai", 1),
            vec![SearchMatch {
                ordinal: 1,
                content: "AI AI infrastructure".into(),
                score: 2
    ...
    fn ranks_related_investment_terms_below_exact_matches() {
        let chunks = vec![
            TextChunk {
                ordinal: 0,
                content: "Revenue increased this quarter".into(),
            },
            TextChunk {
                ordinal: 1,
                content: "Sales increased this quarter".into(),
            },
        ];
