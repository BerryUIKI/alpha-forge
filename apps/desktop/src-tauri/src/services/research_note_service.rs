...
impl ResearchNoteService {
    pub fn new(repo: ResearchNoteRepository) -> Self { Self { repo } }

    pub async fn create_note(&self, input: CreateNoteInput) -> Result<ResearchNote, AppError> {
        if input.content.trim().is_empty() {
            return Err(AppError::Validation("Note content cannot be empty".to_string()));
        }
    ...
    pub async fn delete_note(&self, id: &str) -> Result<(), AppError> { self.repo.delete(id).await }
}
