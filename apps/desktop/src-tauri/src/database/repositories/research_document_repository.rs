...
pub struct ResearchDocumentRepository { pool: SqlitePool }
...
impl ResearchDocumentRepository {
    ...
    pub async fn create(&self, input: CreateDocumentInput) -> Result<ResearchDocument, AppError> {
        ...
        Ok(ResearchDocument { id, project_id: input.project_id, document_type: input.document_type, title: input.title, content: input.content, source_url: input.source_url, file_path: input.file_path, created_at: now, updated_at: now })
    }
    ...
    pub async fn list_by_project(&self, project_id: &str) -> Result<Vec<ResearchDocument>, AppError> {
    ...
    pub async fn delete(&self, id: &str) -> Result<(), AppError> {
        sqlx::query("DELETE FROM research_documents WHERE id = ?").bind(id).execute(&self.pool).await
...
#[derive(sqlx::FromRow)]
struct DocumentRow { id: String, project_id: String, document_type: String, title: String, content: Option<String>, source_url: Option<String>, file_path: Option<String>, created_at: String, updated_at: String }

impl From<DocumentRow> for ResearchDocument {
    fn from(row: DocumentRow) -> Self {
        let document_type = match row.document_type.as_str() { "pdf" => DocumentType::Pdf, "web_page" => DocumentType::WebPage, "note" => DocumentType::Note, "report" => DocumentType::Report, _ => DocumentType::Note };
        ResearchDocument { id: row.id, project_id: row.project_id, document_type, title: row.title, content: row.content, source_url: row.source_url, file_path: row.file_path, created_at: row.created_at.parse().unwrap_or_else(|_| Utc::now()), updated_at: row.updated_at.parse().unwrap_or_else(|_| Utc::now()) }
    }
}